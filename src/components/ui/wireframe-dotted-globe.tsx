...
export default function RotatingEarth({ width = 800, height = 600, className = "" }: RotatingEarthProps) {
  ...
  useEffect(() => {
    ...
    const generateDotsInPolygon = (feature: GeoJSON.Feature, dotSpacing = 16) => {
      ...
      const bounds = d3.geoBounds(feature as GeoJSON.Feature<GeoJSON.Geometry>)
    ...
    const render = () => {
      ...
      if (landFeatures) {
        ...
        // Land outlines
        context.beginPath()
        landFeatures.features.forEach((feature) => path(feature as GeoJSON.Feature))
        context.strokeStyle = "#ffffff"
        context.lineWidth = 1 * scaleFactor
        context.stroke()
    ...
    const loadWorldData = async () => {
      try {
        ...
        landFeatures!.features.forEach((feature) => {
          const dots = generateDotsInPolygon(feature as GeoJSON.Feature, 16)
  ...
  if (error) {
    return (
      <div className={`flex items-center justify-center rounded-2xl p-8 bg-black ${className}`}>
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <canvas
        ...
        style={{ opacity: isLoading ? 0 : 1, transition: "opacity 0.6s ease" }}
      />
    </div>
  )
}
